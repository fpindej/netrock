...
if ! git diff --quiet 2>/dev/null || ! git diff --cached --quiet 2>/dev/null; then
    ...
    WARNINGS="$WARNINGS\nUncommitted changes in $FILE_COUNT file(s). Consider committing before ending."
fi
...
if [ -n "$UNTRACKED" ]; then
    ...
    WARNINGS="$WARNINGS\n$UNTRACKED_COUNT untracked file(s) in src/. May be new files that need committing."
fi

if [ -n "$WARNINGS" ]; then
    # Output as JSON with system message for Claude to see
