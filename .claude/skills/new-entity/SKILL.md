...
1. Create `src/backend/Test.Domain/Entities/{Entity}.cs`:
...
3. Add error messages to `src/backend/Test.Shared/ErrorMessages.cs`
...
5. Add `DbSet<{Entity}>` to `Infrastructure/Persistence/TestDbContext.cs`
6. Run migration:
   ```bash
...
8. `dotnet build src/backend/Test.slnx` - fix errors, loop until green
