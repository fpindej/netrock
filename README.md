...
> **Use the [NETrock Generator](https://netrock.dev) to pick your features and get a ready-to-run project.** No cloning - just choose what you need, download, and start building.
>
...
Skills are also loaded automatically when Claude Code plans work - it reads the relevant skill and follows the procedure without you having to invoke it. The project also includes `CLAUDE.md` and `FILEMAP.md` as structured context files, plus 13 specialized agents (`.claude/agents/`), lifecycle hooks (`.claude/hooks/`) for auto-formatting, safety gates, and quality checks, pinned official plugins (C#/TypeScript LSP, security guidance), and a Playwright MCP server (`.mcp.json`) for browser-level verification. See [`.claude/README.md`](.claude/README.md) for how it fits together. No separate onboarding needed.
