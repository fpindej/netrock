...
> **Start from the env templates:** Copy `deploy/envs/production-example/` to your deployment target, fill in the values, and configure your platform (Docker Compose, Coolify, Kubernetes, etc.) to pass them to the containers.
...
- [ ] **Bootstrap admin** - set `Seed__Users__*` environment variables (see `deploy/envs/production-example/seed.env`) to create an initial SuperAdmin on first deploy. Idempotent - safe to leave set, but remove after creating admin accounts through the UI
