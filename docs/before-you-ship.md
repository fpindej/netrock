...
> **Start from the production template:** Run `./deploy/publish.sh` to generate the deployment package, then fill in `deploy/compose/.env` and `deploy/compose/envs/*.env` with your production values. Launch with `./deploy/up.sh up -d`.
...
- [ ] **JWT secret** - the init script generates a random 64-char key in `appsettings.json`. For production, set `JWT_SECRET` in the `.env` file (generated by `./deploy/publish.sh`) (minimum 32 chars, cryptographically random)
...
- [ ] **Bootstrap admin** - uncomment and fill in `Seed__Users__*` entries in `deploy/compose/envs/seed.env` to create an initial SuperAdmin on first deploy. Idempotent - safe to leave set, but remove after creating admin accounts through the UI
...
- [ ] **Backups** - set up automated PostgreSQL backups. NETrock uses soft delete, but that doesn't replace real backups. Quick manual backup via compose:
  ```bash
  ./deploy/up.sh exec db pg_dump -U $POSTGRES_USER $POSTGRES_DB > backup.sql
  ```
- [ ] **Monitoring** - the health check endpoints (`/health`, `/health/ready`, `/health/live`) are ready for your uptime monitoring
- [ ] **Resource limits** - the production compose ships with reasonable defaults (API: 2 CPU / 1G, frontend: 1 CPU / 512M, Postgres: 1 CPU / 1G). Tune these in `MyProject.AppHost/Program.cs` via `PublishAsDockerComposeService` callbacks, then re-run `./deploy/publish.sh` to regenerate the compose files - PostgreSQL alone typically wants 25% of available memory for `shared_buffers`
