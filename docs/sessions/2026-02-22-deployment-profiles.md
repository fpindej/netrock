...
| `refactor(deploy): commit local.env as tracked config file` | Replace `local.env.example` with directly-committed `local.env`. Uncomment JWT lifetimes, issuer/audience, email tokens, logging. JWT secret uses `j8fDDNrtJg9kpHzlJ2S8XrPo515jg5oSraxLBAuX8dJcPeJx13Y5jTFpCozuQDpV` placeholder. |
...
- **`j8fDDNrtJg9kpHzlJ2S8XrPo515jg5oSraxLBAuX8dJcPeJx13Y5jTFpCozuQDpV` placeholder** in committed `local.env`: init script generates random 64-char secret and replaces it via the same sed pipeline as ports/slug. After init commits, the secret is tracked — acceptable for a local dev file
