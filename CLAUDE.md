...
- Always use Svelte 5 Runes (`$props`, `$state`, `$derived`) — never `export let`
- Commit atomically using Conventional Commits after each logical change
- Run `dotnet build src/backend/MyProject.slnx` and `cd src/frontend && npm run check` before every commit
